import 'package:mukabbir/common/jwt_enum.dart';
import 'package:mukabbir/constant/jwt_decoding/decoding.dart';
import 'package:mukabbir/data/local_data_source/jwt/jwt_local_access_token.dart';
import 'package:mukabbir/data/models/tokens_model.dart';
...
class JwtAccessRepo {
  ...
  Future<String?> get loadAccessToken async {
    ...
    if (!JwtDecoding.tokenHasExpired(accessToken) && !JwtDecoding.tokenHasExpired(refreshToken)) {
      await Future.wait([
        jwtLocalAccessToken.saveToken(SecureStorageKeys.accessToken, accessToken!),
        jwtLocalAccessToken.saveToken(SecureStorageKeys.refreshToken, refreshToken!)
      ]);
